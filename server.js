...
const DIABLO_BANNER = `╭━━━━━━━〔 *${BOT_NAME} V10.41* 〕━━━━━━━⬣\n╰━━━━━━━━━━━━━━⬣`
...
// ========== COMMANDS ==========
cmd('ai','all','Chat with AI', async(sock, msg, args) => {const prompt = args.join(' '); if(!prompt) return sock.sendMessage(msg.key.remoteJid, {text: `❌ Use: ${PREFIX}ai text`}); await sock.sendMessage(msg.key.remoteJid, {text: `${DIABLO_BANNER}\n\nYou: "${prompt}"\nBot: I'm ${BOT_NAME} AI.`})});
...
cmd('approve','master','Approve', async(sock, msg, args) => {const number = args[0]?.replace(/\D/g,''); await db.read(); if(!await isPaired(number)) db.data.paired.push({number: number, pairedBy: MASTER}); db.data.pendingRequests = db.data.pendingRequests.filter(r => r.number!== number); await db.write();});
...
// 180 FILLER
for(let i=1;i<=180;i++){ cmd(`cmd${i}`,'paired',`Utility ${i}`, async(s,m)=>s.sendMessage(m.key.remoteJid,{text:`Cmd ${i}`}))}
...
async function startSession(number) {
    ...
    sock.ev.on('connection.update', async (update) => {const { connection, lastDisconnect } = update; if(connection === 'open') {updateSession.run('connected', null, number)} if(connection === 'close') {sessions.delete(number); const reason = lastDisconnect?.error?.output?.statusCode; if(reason!== DisconnectReason.loggedOut) startSession(number)} if(!state.creds.registered) {const code = await sock.requestPairingCode(number); updateSession.run('waiting', code, number)}});
    ...
    sock.ev.on('messages.upsert', async ({messages}) => {
        ...
        // ANTIBADWORD + ANTILINK CHECK
        if(from.endsWith('@g.us')){
            await db.read();
            // ANTILINK
            if(db.data.antilink?.[from] && (text.includes('chat.whatsapp.com') || text.includes('t.me'))) {
                const quoted = msg.key.id;
                await sock.sendMessage(from, {delete: {remoteJid: from, id: quoted, fromMe: false}});
                await sock.sendMessage(from, {text: `🔗 @${senderNum} Link not allowed`, mentions: [sender]});
            }
            // ANTIBADWORD
            if(db.data.antibadword?.[from]) {
                const foundBad = db.data.badwords.find(word => text.toLowerCase().includes(word));
                if(foundBad) {
                    await sock.sendMessage(from, {delete: {remoteJid: from, id: msg.key.id, fromMe: false}});
                    await sock.sendMessage(from, {text: `🚫 @${senderNum} Bad word detected: "${foundBad}"`, mentions: [sender]});
                    commands.get('warn').run(sock, msg, [senderNum, `Bad word: ${foundBad}`]);
                }
            }
        }
    ...
    sock.ev.on('group-participants.update', async (update) => {
        ...
        if(action === 'add') {
            await db.read();
            if(db.data.antibot?.[id]) {
                for(const participant of participants) {
                    const userNum = jidDecode(participant).user;
                    const isBot = userNum.length > 12 &&!userNum.startsWith(MASTER);
                    if(isBot) {
                        await sock.groupParticipantsUpdate(id, [participant], "remove");
                        await sock.sendMessage(id, {text: `🛡️ ANTIBOT: Kicked bot @${userNum}`, mentions: [participant]});
                        await sock.sendMessage(`${MASTER}@s.whatsapp.net`, {text: `${DIABLO_BANNER}\n\n🚨 BOT KICKED\n*Group:* ${(await sock.groupMetadata(id)).subject}\n*Bot:* 234${userNum}`});
                    }
                }
            }
        }
        if(action === 'promote') {
            ...
            for(const participant of participants) {
                const userNum = jidDecode(participant).user;
                const isPairedUser = await isPaired(userNum);
                if(isPairedUser && userNum!== MASTER) {
                    const groupName = (await sock.groupMetadata(id))