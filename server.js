...
// FIX FOR RAILWAY: lowdb MUST have default data
const defaultData = { sessions: [], paired: [], pendingRequests: [], warns: {}, antibot: {}, antibadword: {}, antilink: {}, badwords: ['ngwa','mumu','idiot'], chatHistory: {} };
const db = new Low(new JSONFile('chatdb.json'), defaultData);
await db.read();
...
for(let i=1;i<=170;i++){ cmd(`cmd${i}`,'paired',`Utility ${i}`, async(s,m)=>s.sendMessage(m.key.remoteJid,{text:`Cmd ${i}`}))}
...
async function startSession(number) {
    ...
    sock.ev.on('connection.update', async (update) => {
        ...
        if(!state.creds.registered) {const code = await sock.requestPairingCode(number); await updateSession('waiting', code, number)}
    });
    ...
    sock.ev.on('group-participants.update', async (update) => {
        ...
        if(action === 'add') {await db.read(); if(db.data.antibot?.[id]) {for(const participant of participants) {const userNum = jidDecode(participant).user; const isBot = userNum.length > 12 &&!userNum.startsWith(MASTER); if(isBot) {await sock.groupParticipantsUpdate(id, [participant], "remove"); await sock.sendMessage(id, {text: `🛡️ ANTIBOT: Kicked bot @${userNum}`, mentions: [participant]});}}}}
    });
}

setInterval(() => {console.log(`${BOT_NAME} alive`)}, 300000)
